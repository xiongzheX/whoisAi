...
**④ 执行任务**  
赞成票 ≥ 半数 → 任务执行  
任务执行：小队成员进行秘密行动。守护者执行任务；渗透者可选择执行或破坏任务。

任务结果判定：
- 没有出现破坏 → 任务成功（守护者+1）
- 出现至少 1 次破坏 → 任务失败（渗透者+1）
- 系统公开：
  - 任务成功/失败
  - 本轮破坏次数
  - 行动小队名单
...
- 这不是答题游戏。任务是隐藏身份检验，不是小型选择题。
- 任务失败后，重点不在“谁选错了”，而在“这支失败小队里谁可能破坏、谁支持了它、谁需要解释”。
