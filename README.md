...
好友模式通过 HTTP 轮询同步，发牌、行动校验、电脑决策和 20 秒限时均由服务端执行，仅返回当前玩家自己的手牌（摊牌除外）。刷新同一标签页可恢复；断线仍会超时自动行动。等待房或结算后离线 90 秒释放席位，并自动转移房主。数据目前仅保存在服务端内存，尚未落库，重启服务会丢失房间和进度。游戏不涉及充值或兑换。单人练习入口为 `/games/texas-holdem/?mode=solo`。电脑根据自己的手牌、公共牌、位置、对手人数、底池赔率及最近 24 次公开操作选择动作，不读取其他玩家手牌或剩余牌堆。八位牌友拥有不同松紧度、攻击性、诈唬频率、下注尺度和思考节奏；这是带随机性的启发式陪练，不是精确胜率或最优策略求解器。单人练习刷新会重置进度；豆豆用完或赢下全桌后，可以重新开桌。
