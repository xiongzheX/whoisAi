...
当前版本包含统一游戏大厅、公开等待房间、邀请链接、断线恢复、原房复玩，以及三款可完整游玩的游戏。所有关键对局结果由 Go 服务端生成，房主浏览器不是权威状态源。
...
平台首页会展示每款游戏当前可加入的等待房间。玩家既可以直接加入，也可以快速开房，不再必须手动传递房间号。

## 本版重点

- 将单一《谁是 AI》项目升级为可承载多款游戏的平台大厅。
- 把长期存在的好友房间 `PartyRoom` 与单局游戏 `GameSession` 分离，支持复玩和切换游戏。
- 接入《豆豆百米赛》《团子相扑》，并改造成两名真人共享房间、分别配置、共同结算的玩法。
- 双人游戏采用服务端权威模拟：双方收到同一个随机环境、种子、胜负结果和回放。
- 补齐公开等待房间、快捷开房、邀请链接、准备高亮、返回大厅、失效房间清理和断线提示。
- 重做大厅与三款游戏的视觉系统，使用明快但不过饱和的配色、清晰字体和移动端布局。
- 优化《谁是 AI》的正常、测试、单人调试模式，减少弹窗打断，并补齐游戏结束后的显著出口。
...
更完整的领域设计、关系表与状态迁移规则见 [社交小游戏平台底层设计](docs/architecture/social-game-platform.md)。PostgreSQL 目标表结构见 [001_platform.sql](internal/platform/migrations/001_platform.sql)。

> 当前本地运行使用 `platform.MemoryStore`。服务重启后等待房间与对局状态会清空；SQL 文件是后续持久化实现的结构基线，并不表示当前已连接 PostgreSQL。
...
- [全游戏体验审查与修复记录（2026-07-22）](docs/reviews/full-game-experience-audit-2026-07-22.md)
- [多用户模拟体验问题记录](docs/reviews/multi-agent-game-experience-2026-07-20.md)
- [真实游戏、测试模式与单人调试体验](docs/reviews/real-game-experience-2026-07-15.md)
- [Parti 项目研究与平台路线启发](docs/reviews/parti-project-study-2026-07-20.md)
- [玩法修复记录](docs/reviews/gameplay-fix-2026-07-15.md)

## 当前边界与下一步

- 房间和 Session 目前存储在内存中，尚未接入 PostgreSQL 持久化。
- 游戏由平台内置注册，暂不支持第三方上传任意游戏代码。
- 下一阶段优先完成客户端动作 `action_id` 的全链路幂等、统一 `GameDriver` 接口和更完整的局后社交复盘。
